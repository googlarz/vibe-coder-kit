...
1. **Check for exposed secrets in the codebase.** Run `/vibe-check`. If vibe-check isn't set up yet, I'll scan silently for common patterns — files containing `sk-`, `AIza`, `AKIA`, `Bearer `, database connection strings with passwords, or anything that looks like `password=`, `secret=`, `api_key=` with a real value next to it. You don't need to read code for this.
